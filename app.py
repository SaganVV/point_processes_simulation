...
class SimulationApp:
    ...
    def get_user_inputs(self):
        ...
        density_select = st.sidebar.selectbox(
            "Select point process",
            ("Poisson", "Hardcore", "Strauss", "Saturated Density"),
        )
    ...
    def _get_normalized_probs(self):
        ...
        for i, state in enumerate(BDM_states):
            with probs_columns[i]:
                probs[i] = st.number_input(
                    f"{state.name}", 0.0, 1.0, 1 / len(BDM_states)
                )
    ...
    def get_simulation_parameters(self):
        ...
        num_iterations = st.sidebar.number_input(
            "Iterations", 0, max_value=None, value=2000
        )
        warm_up = st.sidebar.number_input(
            "Warm Up iterations", 0, max_value=None, value=0
        )
    ...
    def _run_pure_simulation(self, progress_callback=None, **params):
        ...
        bdm = BirthDeathMigration(
            params["density"], state_sampler_probs=params["probs"]
        )
        total_iter = params["num_iterations"] + params["warm_up"]
        for i, state in enumerate(
            bdm.run(
                num_iter=params["num_iterations"],
                warm_up=params["warm_up"],
                callbacks=[tracker, num_of_points],
            )
        ):
    ...
    def _render_point_statistics(self):
        ...
        with col1:
            st.metric(
                "Sample mean of number of points",
                f"{np.mean(history_pd['num_of_points']):.2f}",
            )
        with col2:
        ...
        fig_points = make_subplots(
            ...
            cols=2,
            subplot_titles=(
                "Number of points over iterations",
        ...
        fig_points.add_trace(
            go.Scatter(
                x=history_pd["iter"],
                y=cummean(history_pd["num_of_points"]),
                mode="lines",
    ...
    def _render_acceptance_analysis(self):
        history_pd = st.session_state.history_pd
        fig_accept = go.Figure(
            layout_title="Acceptance probabilities by proposal type",
            layout_yaxis_range=[0, 1],
            layout_xaxis_range=[0, len(history_pd)],
        )
    ...
    def run(self):
        ...
        tab1, tab2, tab3 = st.tabs(
            ["Points", "Number of points statistics", "Acceptance Analysis"]
        )
